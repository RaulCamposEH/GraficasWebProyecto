...
function publicar(score, jugador, modo) {
  FB.api(
    ...
    "POST",
    {
      message: 'El jugador ' + jugador + ' ha obtenido una puntuacion de: ' + score + ' en el modo ' + modo,
      access_token:
...
loader.load('spawn_points_lvl3.glb', (gltf) => {
...
function updateMovableTargets(deltaTime) {
  if (startRoundTimer.timeEnded) {
    if (NUM_OF_ROUNDS * 3 == Round_index) {
...
document.getElementById("btn-start-easy").addEventListener("click", () => {
  ...
  nextTargetTimer = new Temporizador(0.3);
  levelTimer.start()
...
document.getElementById("btn-start-hard").addEventListener("click", () => {
  ...
  nextTargetTimer = new Temporizador(0.15);
  levelTimer.start()
  document.getElementById("modal").classList.toggle("hideElement")
  document.body.requestPointerLock();
})
...
document.getElementById("btn-send").addEventListener("click", () => {
  ...
  publicar(score, name, mode)
  setTimeout(() => window.location.reload(), 1000)
})

loader.load('Scene03.glb', async (gltf) => {
